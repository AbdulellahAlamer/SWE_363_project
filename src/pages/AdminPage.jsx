...
export default function AdminPage() {
  const [clubs, setClubs] = useState([
    { id: 'c1', initials: 'CS', name: 'Computer Club', updated: 'Updated 2d ago', president: 'Saeed Al-Qahtani', members: 860 },
    { id: 'c2', initials: 'IE', name: 'Industrial Engineering Society', updated: 'Updated 1w ago', president: 'Vacant', members: 320 },
    { id: 'c3', initials: 'ME', name: 'Mechanical Engineers', updated: 'Updated 3d ago', president: 'Sara Al-Otaibi', members: 540 },
  ]);
  ...
  const onDelete = (id) => {
    ...
    if (!club) return;
    const confirmed = window.confirm(`Delete "${club.name}"? This action cannot be undone.`);
    if (confirmed) {
  ...
  const onSaveEdit = (id) => {
    setClubs((prev) => prev.map((c) => (c.id === id ? { ...c, name: editForm.name, president: editForm.president } : c)));
    onCancelEdit();
  };
  return (
      ...
      <div className="flex-1 ml-64">
                  ...
                  {clubs
                    ...
                    <tr key={club.id} className="border-b border-gray-200">
                      <td className="py-4 flex items-center gap-3">
                        <div className="w-8 h-8 bg-blue-100 rounded-lg flex items-center justify-center text-blue-600">{club.initials}</div>
        ...
        <div className="mx-8 bg-white rounded-lg shadow-md p-6">
          ...
          <div className="mb-6">
            ...
            >
              ...
              {logoPreview ? (
                ...
                <>
                  <svg className="w-8 h-8 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M7 16V4m0 0L3 8m4-4 4 4M17 8v12m0 0l4-4m-4 4-4-4"></path></svg>
                  <div className="text-sm text-gray-500">Click or tap to upload a logo (or take a photo)</div>
          ...
          <div className="flex justify-end gap-4">
            <button onClick={resetForm} className="px-4 py-2 text-gray-600 hover:bg-gray-50 rounded-lg">Reset</button>
            <button className="px-4 py-2 text-blue-600 hover:bg-blue-50 rounded-lg">Save Draft</button>
            <button onClick={publishClub} className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">Publish Club</button>
          </div>
        </div>
